  ...
  },
 {
  ...
  formule: "C₁₅H₁₅N₃O₂",
  categorie: "Indicateur coloré / 4,4<pH<6,2",
  localisation: "salle B27 / étagère D4",
  pictogramme: ["SGH07_PointExclamation.jpg"],
  obligation: ["LUNETTES", "GANTS", "BLOUSE"],
...
// À appeler depuis votre renderTable() existant, pour chaque <tr> :
//   tr.innerHTML = `<td><input type="checkbox" class="row-chk" data-cas="${p.cas}"></td> ...`
 
function toggleSelectAll(master) {
  document.querySelectorAll('.row-chk').forEach(chk => chk.checked = master.checked);
}
 
function getSelectedCas() {
...
function openLabelModal() {
  ...
  document.getElementById('opt-selected').disabled = (sel.length === 0);
  if (sel.length === 0) {
...
function closeLabelModalOutside(e) {
...
function printLabels() {
  ...
  let subset;
  if (scope === 'selected') {
    const cas = getSelectedCas();
    subset = products.filter(p => cas.includes(p.cas));
  } else {
    subset = products;
  }
  ...
  /* Appel vers le générateur dédié (print-labels.js) */
