...
function afficherModeOperatoire(type) {
  ...
  const dilutionEl  = document.getElementById("modeDilution");
  if (!dissolution || !dilutionEl) return;

  if (type === "dissolution") {
    dissolution.classList.remove("hidden");
    dilutionEl.classList.add("hidden");
  } else {
    dissolution.classList.add("hidden");
    dilutionEl.classList.remove("hidden");
  }
}
...
/* ==========================================================
   REMPLISSAGE LISTE RÉACTIFS
   ========================================================== */
function remplirListeReactifs() {
  const select = $("reactif");
  if (!select) return;

  const categories = [...document.querySelectorAll(".filtre-cat:checked")].map(cb => cb.value);
  ...
  products
    .filter(p => categories.some(cat => appartientCategorie(p, cat)))
    .sort((a, b) => a.nom.localeCompare(b.nom))
    .forEach(p => {
...
/* ==========================================================
   CHANGEMENT DE RÉACTIF (dissolution)
   ========================================================== */
...
function calculEcart() {
  ...
  if (masseExp <= 0 || C <= 0 || V <= 0 || M <= 0) {
    if (zoneEcart) zoneEcart.textContent = "—";
...
function _lancerCompteRendu() {
  ...
  const sections = [
    ...
    {
      groupe: "dissolution",
      titre: "Qu'est-ce qu'une dissolution ? Qu'est-ce qu'une dilution ? Expliquer la différence.",
      competence: "APP",
      notation: true,
    ...
    {
      groupe: "dissolution",
      titre: "Pourquoi faut-il toujours verser l'acide dans l'eau et jamais l'inverse ?",
      competence: "APP",
      notation: true,
    ...
    {
      groupe: "dissolution",
      titre: "Convertir le volume V utilisé de mL en L.",
      competence: "REA",
      notation: true,
    ...
    {
      groupe: "dissolution",
      titre: "Calculer la quantité de matière n du réactif à partir de la masse théorique et de M.",
      competence: "REA",
      notation: true,
    ...
    {
      groupe: "dissolution",
      titre: "Convertir la masse théorique calculée de g en mg.",
      competence: "REA",
      notation: true,
    ...
    {
      groupe: "dissolution",
      titre: "Calculer la concentration massique Cm (g/L) de la solution préparée.",
      competence: "REA",
      notation: true,
    ...
    {
      groupe: "dissolution",
      titre: "Calculer l'erreur absolue Δm entre la masse théorique et la masse pesée.",
      competence: "ANA RAI",
      notation: true,
    ...
    {
      groupe: "dissolution",
      titre: "Calculer l'erreur relative (%). Cette erreur est-elle acceptable (seuil 2 %) ?",
      competence: "ANA RAI",
      notation: true,
    ...
    {
      groupe: "dissolution",
      titre: "Identifier les sources d'erreurs expérimentales et proposer des améliorations.",
      competence: "VAL",
      notation: true,
    ...
    {
      groupe: "dissolution",
      titre: "Rédiger une conclusion synthétique (nature, concentration molaire, qualité de la pesée).",
      competence: "COM",
      notation: true,
...
function initRadarCompetences() {
  ...
  document
    ...
    .forEach(radio => radio.addEventListener("change", afficherRadarCompetences));
}
...
function afficherRadarCompetences() {
  ...
  zone.innerHTML = `<canvas id="canvasRadarCompetences" width="420" height="420"></canvas>`;
