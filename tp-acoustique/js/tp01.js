...
function drawScope() {
  ...
  const d   = +document.getElementById('distRange').value / 100; // m
  const tau = d / V_SON; // s
  const T   = 1 / f;
  const phi = (tau / T) * 2 * Math.PI; // déphasage CH2
  ...
  document.getElementById('scopeReadout').textContent =
    `f = ${f.toLocaleString('fr')} Hz · d = ${(d*100).toFixed(1)} cm`;
  document.getElementById('deltaDisplay').textContent =
    `Δt = ${(tau*1000).toFixed(2)} ms · λ = ${(V_SON/f*100).toFixed(1)} cm`;
